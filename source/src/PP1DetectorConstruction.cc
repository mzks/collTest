...
//  G4VSolid* blank = new G4Box("Blank",
  ...
  new G4PVPlacement(0,			//no rotation
	...
	G4double collimatorLength = source_length - source_offset - 0.1*mm;
	G4double collimatorPosition = collimatorLength / 2.0  + source_offset;
  ...
  auto logicCollimatorHole = new G4LogicalVolume(solidCollimatorHole,		//its solid
	...
	new G4PVPlacement(
			G4Transform3D(mat_source,G4ThreeVector(collimatorPosition,0,0)),
			"logicCollimator",
			logicCollimator,
		   	physWorld, 
			false,
